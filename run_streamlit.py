...
from prompts import get_system_prompt, get_system_prompt_with_site_text, get_system_prompt_only_feedback, \
    get_system_prompt_only_conversation
import hmac
...
def get_recordings():
    with st.container():
        ...
        if audio_bytes and audio_bytes != st.session_state["last_recorder"]:
            st.session_state["last_recorder"] = audio_bytes
...
if "last_recorder" not in st.session_state:
    st.session_state["last_recorder"] = None
...
if st.session_state["file_uploaded"] is not None:
    ...
    if st.session_state["position_text"]:
        ...
        st.session_state.messages.append({"role": "system", "content": get_system_prompt_only_conversation(cv_text)})
            ...
            if prompt or audio_prompt:
                ...
                with st.chat_message("assistant"):
                    with st.spinner("Thinking what to answer..."):
                        ...
                        feedback_answer = get_text_from_openai(get_system_prompt_only_feedback())
